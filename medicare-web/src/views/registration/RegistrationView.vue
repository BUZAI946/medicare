...
<<<<<<< HEAD
              <div style="display:flex;gap:8px;align-items:center">
                ...
                <el-select v-model="querySlot" placeholder="时段" clearable size="small" style="width:100px" @change="loadSchedules">
...
async function generateSchedules() {
  ...
  try {
    ...
    // 先加载已有排班，避免重复创建
    const [existToday, existTomorrow] = await Promise.all([
      getAvailableSchedules(today), getAvailableSchedules(tomorrow)
    ])
    const existing = new Set<string>()
    ;[...(existToday.data||[]), ...(existTomorrow.data||[])].forEach((s: any) => {
      existing.add(`${s.doctorId}-${s.workDate}-${s.timeSlot}`)
    })

    let count = 0, skipped = 0
    const slots = ['上午(08:00-12:00)', '下午(14:00-17:30)']
    for (const d of doctors.data) {
      for (const date of [today, tomorrow]) {
        for (const slot of slots) {
          const key = `${d.id}-${date}-${slot}`
          if (existing.has(key)) { skipped++; continue }
          try {
            await createSchedule({ doctorId: d.id!, workDate: date, timeSlot: slot, totalSlots: 20, remainSlots: 20 })
            count++
          } catch {}
        }
      }
    }
    if (count > 0) ElMessage.success(`已生成 ${count} 条排班${skipped > 0 ? `，跳过 ${skipped} 条已有` : ''}`)
    else ElMessage.info(`排班已存在，无需重复生成（${skipped}条）`)
    loadSchedules()
  } catch {}
