<template>
  <el-container class="main-layout">
    ...
    <!-- 首次登录引导 -->
    <el-dialog v-model="showGuide" title="👋 欢迎使用 MediCare" width="500px" :close-on-click-modal="false" :show-close="false">
      <div class="guide-steps">
        <div class="guide-step" v-for="(s, i) in guideSteps" :key="i">
      ...
      <template #footer>
        <el-button type="primary" size="large" @click="closeGuide" style="width:100%">我知道了，开始使用</el-button>
      </template>
    </el-dialog>
  </el-container>
</template>
...
const allMenuItems = [
  ...
  { path: '/datav', title: '数据大屏', icon: 'Monitor', roles: ['admin', 'doctor', 'pharmacist'] },
  ...
  { path: '/calendar', title: '预约日历', icon: 'Calendar', roles: ['admin', 'doctor'] },
  { path: '/reports', title: '报表中心', icon: 'DataAnalysis', roles: ['admin'] },
  { path: '/payments', title: '收费管理', icon: 'Money', roles: ['admin'] },
...
// 首次登录引导
const showGuide = ref(!localStorage.getItem('medicare-guided'))
const guideSteps = [
  { title: '挂号预约', desc: '在挂号页面挂号 → 选择科室 → 选择医生 → 确认，患者进入候诊队列' },
  { title: '医生工作站', desc: '叫号 → 书写病历 → AI智能分析辅助诊断 → 保存病历' },
  { title: '开立处方', desc: '在处方页面选择已就诊患者 → 智能推荐药品 → 开方，药剂师取药确认' },
  { title: '数据大屏', desc: '候诊大厅展示实时数据，报表中心查看营收和工作量统计' },
]
function closeGuide() { showGuide.value = false; localStorage.setItem('medicare-guided', '1') }
