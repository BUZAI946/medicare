...
public class MedicineController {
    ...
    public Result<List<Medicine>> list(@RequestParam(required = false) String keyword) {
        return Result.ok(medicineService.findAll(keyword));
    }
    ...
    public Result<Void> stockIn(@PathVariable Long id, @Valid @RequestBody StockRequest request) {
        ...
        if (request.getExpiryDate() != null && !request.getExpiryDate().isBlank()) {
            expireDate = LocalDate.parse(request.getExpiryDate()).atStartOfDay();
        }
        medicineService.stockIn(id, request.getQuantity(), request.getBatchNo(), expireDate);
        return Result.ok();
    }
